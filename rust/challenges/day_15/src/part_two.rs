...
fn main() {
    ...
    println!("[Test] 315 = {}", solve(&test_values));
...
fn solve(data: &Vec<Vec<usize>>) -> usize {
    let bigger_data = embiggen(data);

    let dest = (bigger_data.len() - 1, bigger_data.last().unwrap().len() - 1);

    let graph = Graph::from_vec(&bigger_data);
    let paths = graph.distances((0, 0));

    *paths.get(&dest).unwrap()
}

fn embiggen(data: &Vec<Vec<usize>>) -> Vec<Vec<usize>> {
    let mut bigger_data: Vec<Vec<usize>> = Vec::new();
    for i in 0..5 {
        for row in data.iter() {
            let new_row: Vec<usize> = (0..5)
                .flat_map(|j| {
                    row.iter().map(move |&x| {
                        let new_value = (x + i + j) % 9;
                        if new_value == 0 {
                            9
                        } else {
                            new_value
