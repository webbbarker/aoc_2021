...
fn main() {
    ...
    println!("[Test] 1924 = {}", solve(&mut test_values));
    ...
    if let Ok(live_data) = read_lines("challenges/day_4/input/input.txt") {
        ...
        println!("[PartTwo] {}", solve(&mut live_values));
    }
}
...
impl Game {
    ...
    fn play_to_lose(&mut self) -> u64 {
        for draw in &self.draws {
            let board_count = self.boards.iter().filter(|&b| !b.is_winner()).count();
            for board in self.boards.iter_mut().filter(|b| !b.is_winner()) {
                if board.mark_draw(*draw) && board_count == 1 {
                    let unmatched_numbers = board.unmatch_numbers();
                    let remaining_points = unmatched_numbers.iter().sum::<u64>();
                    return remaining_points * draw;
                }
            }
        }
