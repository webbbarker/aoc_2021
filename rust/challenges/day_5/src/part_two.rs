...
fn main() {
    ...
    println!("[Test] 12 = {}", solve(&test_values));
    ...
    if let Ok(live_data) = read_lines("challenges/day_5/input/input.txt") {
        ...
        let live_values: Vec<((u64, u64), (u64, u64))> = raw_data
            .iter()
            .map(|s| {
                let parts: Vec<&str> = s.split(" -> ").collect();
                let points: Vec<u64> = parts
...
fn solve(data: &[((u64, u64), (u64, u64))]) -> u64 {
    let mut covered_points: HashMap<(u64, u64), i32> = HashMap::new();
    for range in data {
        let p1 = range.0;
        let p2 = range.1;
        ...
        let mut pos = p1;
        while pos != p2 {
            *covered_points.entry(pos).or_insert(0) += 1;
            let (x1, y1) = pos;
            let (x2, y2) = p2;
        ...
        *covered_points.entry(p2).or_insert(0) += 1;
    }
